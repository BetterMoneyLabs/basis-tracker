...
curl -s "http://localhost:3000/events?page=1&page_size=2" | jq '.data[] | {type, timestamp, amount, collateral_amount}' 2>/dev/null || curl -s "http://localhost:3000/events?page=1&page_size=2"
echo -e "\n"

# Test paginated events with just page parameter
echo "4. Testing paginated events (page 0, size 5):"
curl -s "http://localhost:3000/events?page=0&page_size=5" | jq '.data[] | {type, issuer_pubkey, recipient_pubkey, reserve_box_id}' 2>/dev/null || curl -s "http://localhost:3000/events?page=0&page_size=5"
echo -e "\n"
