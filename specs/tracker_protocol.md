...
See:
- `crates/basis_server/src/api.rs` — HTTP API for note and tracker state operations
- `crates/basis_server/src/tracker_box_updater.rs` — On-chain tracker commitment box monitoring
- `crates/basis_store/src/tracker_scanner.rs` — Blockchain scanning and reserve event processing
- `crates/basis_store/src/reserve_tracker.rs` — Reserve and debt state tracking
- `crates/basis_trees/src/` — AVL+ tree storage, proofs, and state commitment
- `contract/basis.es` — On-chain basis contract that verifies tracker signatures and AVL proofs
