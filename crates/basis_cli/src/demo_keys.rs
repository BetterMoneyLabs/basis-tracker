...
pub fn alice() -> DemoParticipant {
    ...
    participant_from_secret(
        "alice",
        &secret_hex,
        "9hNQcqi72NB5u5Tw6tbfCGbEKByguR7njvcyZXnXPLvV3Do1DiJ",
    )
}
...
pub fn tracker() -> DemoParticipant {
    ...
    participant_from_secret(
        "tracker",
        &secret_hex,
        "9f7ZXamnfaDZL7EWLKLuBZgWMuHCusQYK6yow2d7p2eES9oRRRe",
    )
}
...
mod tests {
    ...
    fn test_demo_keys_have_valid_keys() {
        ...
        assert!(
            alice.public_key().serialize()[0] == 0x02 || alice.public_key().serialize()[0] == 0x03
        );
        assert!(bob.public_key().serialize()[0] == 0x02 || bob.public_key().serialize()[0] == 0x03);
        assert!(
            tracker.public_key().serialize()[0] == 0x02
                || tracker.public_key().serialize()[0] == 0x03
        );
    }
    ...
    fn test_demo_keys_are_distinct() {
        ...
        assert_ne!(
            alice.public_key().serialize(),
            tracker.public_key().serialize()
        );
        assert_ne!(
            bob.public_key().serialize(),
            tracker.public_key().serialize()
        );
    }
    ...
    fn test_demo_keys_match_scala_demo() {
        ...
        assert_eq!(
            alice.public_key().serialize().len(),
            33,
            "Alice pubkey must be 33 bytes"
        );
        assert!(
            alice.public_key().serialize()[0] == 0x02 || alice.public_key().serialize()[0] == 0x03,
        ...
        assert_eq!(
            bob.public_key().serialize().len(),
            33,
            "Bob pubkey must be 33 bytes"
        );
        assert!(
            bob.public_key().serialize()[0] == 0x02 || bob.public_key().serialize()[0] == 0x03,
            "Bob pubkey must be compressed format"
        );

        assert_eq!(
            tracker.public_key().serialize().len(),
            33,
            "Tracker pubkey must be 33 bytes"
        );
        assert!(
            tracker.public_key().serialize()[0] == 0x02
                || tracker.public_key().serialize()[0] == 0x03,
