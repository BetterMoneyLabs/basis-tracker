...
impl TrackerClient {
    ...
    pub async fn get_issuer_notes(&self, pubkey: &str) -> Result<Vec<SerializableIouNote>> {
        ...
        if response.status() == 200 {
            ...
            let error_text = response.into_string()?;
            Err(anyhow::anyhow!("Failed to get issuer notes: {}", error_text))
        }
    }
    ...
    pub async fn get_recipient_notes(&self, pubkey: &str) -> Result<Vec<SerializableIouNote>> {
        ...
        if response.status() == 200 {
            ...
            let error_text = response.into_string()?;
            Err(anyhow::anyhow!("Failed to get recipient notes: {}", error_text))
        }
    }

    pub async fn get_note(&self, issuer: &str, recipient: &str) -> Result<Option<SerializableIouNote>> {
        let url = format!("{}/notes/issuer/{}/recipient/{}", self.base_url, issuer, recipient);
        let response = ureq::get(&url).call()?;
    ...
    pub async fn get_reserve_status(&self, pubkey: &str) -> Result<KeyStatusResponse> {
        ...
        if response.status() == 200 {
            ...
            let error_text = response.into_string()?;
            Err(anyhow::anyhow!("Failed to get reserve status: {}", error_text))
        }
    }
    ...
    pub async fn initiate_redemption(&self, request: RedeemRequest) -> Result<RedeemResponse> {
        ...
        if response.status() == 200 {
            ...
            let error_text = response.into_string()?;
            Err(anyhow::anyhow!("Failed to initiate redemption: {}", error_text))
        }
    }
    ...
    pub async fn complete_redemption(&self, request: CompleteRedemptionRequest) -> Result<()> {
        ...
        if response.status() == 200 {
            ...
            let error_text = response.into_string()?;
            Err(anyhow::anyhow!("Failed to complete redemption: {}", error_text))
        }
    }

    // Events & Status
    pub async fn get_events(&self, page: usize, page_size: usize) -> Result<Vec<TrackerEvent>> {
        let url = format!("{}/events/paginated?page={}&page_size={}", self.base_url, page, page_size);
        let response = ureq::get(&url).call()?;
