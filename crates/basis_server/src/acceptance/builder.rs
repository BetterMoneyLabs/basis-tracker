    ...
    fn build_predicate(&mut self, name: &str) -> Result<Box<dyn NotePredicate>, BuilderError> {
        ...
        let result = match config {
            ...
            PredicateConfig::Blacklist { name, holders } => {
                let parsed_holders = Self::parse_holders(&holders)?;
                Ok(Box::new(BlacklistPredicate::new(name, parsed_holders))
                    as Box<dyn NotePredicate>)
            }
            PredicateConfig::Collateralization { name, min_ratio } => {
                Ok(Box::new(CollateralizationPredicate::new(name, min_ratio))
                    as Box<dyn NotePredicate>)
            }
        ...
        if let Ok(ref pred) = result {
            self.predicates
                .insert(name.to_string(), Self::clone_predicate(pred.as_ref()));
        }
...
mod tests {
    ...
    fn test_build_blacklist() {
        let config = AcceptanceConfig {
            root: None,
            default: DefaultPolicy::Reject,
            predicates: vec![PredicateConfig::Blacklist {
    ...
    fn test_missing_reference() {
        let config = AcceptanceConfig {
            root: None,
            default: DefaultPolicy::Reject,
            predicates: vec![PredicateConfig::AllOf {
                name: "bad".to_string(),
        ...
        let config = AcceptanceConfig {
            ...
            predicates: vec![
                PredicateConfig::Whitelist {
                    ...
                    holders: vec![test_pubkey_hex(1), test_pubkey_hex(2), test_pubkey_hex(3)],
    ...
    fn test_builder_error_display() {
        ...
        assert_eq!(
            format!("{}", err),
            "Invalid public key length: expected 33 bytes, got 5"
        );
    }
