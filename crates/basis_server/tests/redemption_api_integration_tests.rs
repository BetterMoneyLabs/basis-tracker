...
mod redemption_api_tests {
    ...
    use basis_server::{
        api::{complete_redemption, get_redemption_proof, initiate_redemption, prepare_redemption, request_tracker_signature},
        models::{CompleteRedemptionRequest, RedeemRequest, RedemptionPreparationRequest, TrackerSignatureRequest},
    ...
    async fn test_redeem_invalid_hex_pubkey() {
        ...
        let request = RedeemRequest {
            issuer_pubkey: "010101010101010101010101010101010101010101010101010101010101010101".to_string(),
        ...
        let response = initiate_redemption(
    ...
    async fn test_redeem_invalid_pubkey_length() {
        ...
        let request = RedeemRequest {
            ...
            recipient_pubkey: "020202020202020202020202020202020202020202020202020202020202020202".to_string(),
            amount: 1000,
            timestamp: 1234567890,
            reserve_box_id: "".to_string(),
            recipient_address: "".to_string(),
        ...
        let body = &response.1;
        assert!(!body.success || body.data.is_none(),
            "Expected failure or no data, got: {:?}", body);
    }
    ...
    async fn test_redeem_emergency_flag_structure() {
        ...
        if !body.success {
            ...
            assert!(
                error_msg.contains("reserve") || error_msg.contains("Reserve") || error_msg.contains("No matching reserve"),
                "Expected reserve-related error, got: {}", error_msg
            );
        }
    }
    ...
    async fn test_complete_redemption_invalid_hex() {
        ...
        let response = complete_redemption(
            axum::extract::State(state),
            axum::extract::Json(request),
        ).await;
    ...
    async fn test_get_redemption_proof_invalid_hex() {
        ...
        params.insert("recipient_pubkey".to_string(), "020202020202020202020202020202020202020202020202020202020202020202".to_string());

        let response = get_redemption_proof(
            axum::extract::State(state),
    ...
    async fn test_get_redemption_proof_wrong_length() {
        ...
        let mut params = std::collections::HashMap::new();
        // 32 bytes instead of 33
        params.insert("issuer_pubkey".to_string(), "0101010101010101010101010101010101010101010101010101010101010101".to_string());
        params.insert("recipient_pubkey".to_string(), "020202020202020202020202020202020202020202020202020202020202020202".to_string());
    ...
    async fn test_request_tracker_signature_invalid_hex() {
        ...
        let response = request_tracker_signature(
            axum::extract::State(state),
            axum::extract::Json(request),
        ).await;
    ...
    async fn test_request_tracker_signature_valid_structure() {
        ...
        if !body.success {
            ...
            let error_msg = body.error.as_ref().unwrap_or(&default_msg);
            assert!(
                error_msg.contains("tracker") || error_msg.contains("Tracker") || error_msg.contains("sign") || error_msg.contains("node"),
                "Expected tracker/signing-related error for valid request structure, got: {}", error_msg
            );
        }
    }
    ...
    async fn test_prepare_redemption_invalid_hex() {
        ...
        let response = prepare_redemption(
            axum::extract::State(state),
            axum::extract::Json(request),
        ).await;
    ...
    async fn test_redeem_request_deserialization() {
        ...
        assert_eq!(request.issuer_pubkey, "010101010101010101010101010101010101010101010101010101010101010101");
        assert_eq!(request.recipient_pubkey, "020202020202020202020202020202020202020202020202020202020202020202");
