...
use crate::{IouNote, NoteKey, PubKey, NoteError};
...
impl NoteStorage {
    ...
    pub fn open<P: AsRef<Path>>(path: P) -> Result<Self, NoteError> {
        ...
        let partition = keyspace.open_partition("iou_notes", PartitionCreateOptions::default())
    ...
    pub fn get_note(&self, issuer_pubkey: &PubKey, recipient_pubkey: &PubKey) -> Result<Option<IouNote>, NoteError> {
        ...
        match self.partition.get(&key_bytes) {
            Ok(Some(value_bytes)) => {
                ...
                if value_bytes.len() != 33 + 8 + 8 + 64 + 33 {
                    return Err(NoteError::StorageError("Invalid stored note format".to_string()));
                }
                
                let mut offset = 0;
                let _stored_issuer_pubkey: PubKey = value_bytes[offset..offset+33].try_into().unwrap();
                offset += 33;
                
                let amount = u64::from_be_bytes(value_bytes[offset..offset+8].try_into().unwrap());
                offset += 8;
            ...
            Ok(None) => Ok(None),
            Err(e) => Err(NoteError::StorageError(format!("Failed to get note: {}", e))),
        }
    }
    
    /// Remove an IOU note by issuer and recipient public keys
    pub fn remove_note(&self, issuer_pubkey: &PubKey, recipient_pubkey: &PubKey) -> Result<(), NoteError> {
        let key = NoteKey::from_keys(issuer_pubkey, recipient_pubkey);
        let key_bytes = key.to_bytes();
    ...
    pub fn get_issuer_notes(&self, issuer_pubkey: &PubKey) -> Result<Vec<IouNote>, NoteError> {
        ...
        for item in self.partition.iter() {
            let (_key_bytes, value_bytes) = item
                .map_err(|e| NoteError::StorageError(format!("Failed to iterate partition: {}", e)))?;
