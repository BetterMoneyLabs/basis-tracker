...
fn test_comprehensive_schnorr_operations() {
    ...
    let pubkey_from_hex = schnorr::pubkey_from_hex(&hex_pubkey).expect("Failed to parse hex pubkey");
    assert_eq!(alice_pubkey, pubkey_from_hex);
    
    let hex_signature = schnorr::signature_to_hex(&signature);
    let signature_from_hex = schnorr::signature_from_hex(&hex_signature)
