...
fn test_time_lock_validation() {
    ...
    assert!(!recent_redeemable, "Recent note should not be redeemable yet");
...
fn test_signature_verification() {
    ...
    assert!(!wrong_note_valid, "Wrong note should fail with correct issuer");
...
fn test_redemption_request_structure() {
    ...
    println!("Recipient pubkey: {}", &redemption_request.recipient_pubkey[..16]);
...
fn test_simulated_blockchain_data() {
    ...
    let issuer_pubkey_bytes = secp256k1::PublicKey::from_secret_key(&secp, &issuer_secret_key).serialize();
    let tracker_pubkey_bytes = secp256k1::PublicKey::from_secret_key(&secp, &tracker_secret_key).serialize();
    
    let issuer_sig = schnorr::schnorr_sign(&message, &issuer_secret_key, &issuer_pubkey_bytes).unwrap();
    let tracker_sig = schnorr::schnorr_sign(&message, &tracker_secret_key, &tracker_pubkey_bytes).unwrap();
    
    // Verify signatures
    let issuer_valid = schnorr::schnorr_verify(&issuer_sig, &message, &issuer_pubkey_bytes).is_ok();
    let tracker_valid = schnorr::schnorr_verify(&tracker_sig, &message, &tracker_pubkey_bytes).is_ok();
