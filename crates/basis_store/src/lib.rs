...
impl IouNote {
    ...
    pub fn create_and_sign(
        ...
        let message = self::signing_message(&recipient_pubkey, amount, timestamp);
    ...
    pub fn verify_signature(&self, issuer_pubkey: &PubKey) -> Result<(), NoteError> {
        ...
        schnorr_verify(&self.signature, &message, issuer_pubkey)
    }
}
...
fn signing_message(recipient_pubkey: &PubKey, amount: u64, timestamp: u64) -> Vec<u8> {
    let mut message = Vec::new();
...
fn schnorr_sign(message: &[u8], secret_key: &secp256k1::SecretKey, issuer_pubkey: &PubKey) -> Signature {
    use secp256k1::{Secp256k1, SecretKey};
    ...
    let z_bytes = z_scalar.to_be_bytes();
...
fn schnorr_verify(signature: &Signature, message: &[u8], issuer_pubkey: &PubKey) -> Result<(), NoteError> {
