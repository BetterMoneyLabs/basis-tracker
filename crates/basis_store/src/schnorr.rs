...
pub fn signature_to_hex(signature: &Signature) -> String {
    basis_core::impls::signature_to_hex(signature)
}
...
pub fn schnorr_sign(
    ...
    match basis_core::impls::schnorr_sign(message, secret_key_bytes, issuer_pubkey) {
...
pub fn schnorr_verify(
    ...
    match basis_core::impls::schnorr_verify(signature, message, issuer_pubkey) {
        Ok(()) => Ok(()),
        Err(basis_core::traits::CryptoError::InvalidSignature) => Err(NoteError::InvalidSignature),
        Err(basis_core::traits::CryptoError::InvalidPublicKey) => Err(NoteError::InvalidSignature),
        Err(basis_core::traits::CryptoError::InvalidSignatureFormat) => Err(NoteError::InvalidSignature),
...
// Re-export the tests from basis_core
#[cfg(test)]
mod tests {
    use super::*;
    use basis_core::impls::SchnorrVerifier;
    ...
    fn test_roundtrip_signature() {
        ...
        // Create signature
        let signature = schnorr_sign(&message, &secret_key.secret_bytes(), &issuer_pubkey)
