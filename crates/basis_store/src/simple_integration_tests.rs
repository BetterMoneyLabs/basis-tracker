...
impl SimpleIntegrationTestSuite {
    ...
    pub async fn test_basic_functionality(&mut self) -> Result<(), ScannerError> {
        ...
        assert!(boxes.is_empty(), "Mock implementation should return empty boxes");
        ...
        for event in events {
            match event {
                ...
                ReserveEvent::ReserveToppedUp { box_id, additional_collateral, height } => {
                    println!("Mock reserve topped up: {} with additional {} nanoERG at height {}", 
                ...
                ReserveEvent::ReserveRedeemed { box_id, redeemed_amount, height } => {
                    println!("Mock reserve redeemed: {} with {} nanoERG at height {}", 
    ...
    pub async fn test_scanner_state(&self) -> Result<(), ScannerError> {
        ...
        assert!(last_scanned >= 0, "Last scanned height should be non-negative");
