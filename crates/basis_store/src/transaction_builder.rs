...
//!
//! Context Extension Variables (following the deployed reserve contract):
...
impl ContextExtension {
    ...
    pub fn to_json_map(&self) -> HashMap<String, serde_json::Value> {
        ...
        map.insert(
            "0".to_string(),
            serde_json::Value::Number(self.action.into()),
        );

        // #1: Receiver pubkey (hex-encoded)
        map.insert(
            "1".to_string(),
            serde_json::Value::String(hex::encode(&self.receiver_pubkey)),
        );

        // #2: Reserve signature (hex-encoded)
        map.insert(
            "2".to_string(),
            serde_json::Value::String(hex::encode(&self.reserve_signature)),
        );

        // #3: Total debt (number)
        map.insert(
            "3".to_string(),
            serde_json::Value::Number(self.total_debt.into()),
        );

        // #5: Insert proof (hex-encoded)
        map.insert(
            "5".to_string(),
            serde_json::Value::String(hex::encode(&self.insert_proof)),
        );

        // #6: Tracker signature (hex-encoded)
        map.insert(
            "6".to_string(),
            serde_json::Value::String(hex::encode(&self.tracker_signature)),
        );
        ...
        if let Some(ref proof) = self.reserve_lookup_proof {
            map.insert(
                "7".to_string(),
                serde_json::Value::String(hex::encode(proof)),
            );
        }
        ...
        map.insert(
            "8".to_string(),
            serde_json::Value::String(hex::encode(&self.tracker_lookup_proof)),
        );
...
///
/// - Inputs: [Reserve box, fee input boxes...] (spent)
/// - Data Inputs: [Tracker box] (for AVL proof verification)
/// - Outputs: [Updated reserve box, Redemption output box, fee recipient output, change box (optional)]
...
impl RedemptionTransactionBuilder {
    ...
    pub fn build_unsigned_redemption_transaction(
        ...
        if reserve_box_id.is_empty() {
            return Err(TransactionBuilderError::Configuration(
                "Reserve box ID is required".to_string(),
            ));
        }
        ...
        if tracker_box_id.is_empty() {
            return Err(TransactionBuilderError::Configuration(
                "Tracker box ID is required".to_string(),
            ));
        }
        ...
        if tracker_nft_id.is_empty() {
            return Err(TransactionBuilderError::Configuration(
                "Tracker NFT ID is required".to_string(),
            ));
        }
        ...
        if avl_proof.is_empty() {
            return Err(TransactionBuilderError::Configuration(
                "AVL proof is required".to_string(),
            ));
        }
        ...
        if issuer_sig.len() != 65 {
            return Err(TransactionBuilderError::Configuration(
                "Issuer signature must be 65 bytes".to_string(),
            ));
        }

        if tracker_sig.len() != 65 {
            return Err(TransactionBuilderError::Configuration(
                "Tracker signature must be 65 bytes".to_string(),
            ));
        }
        ...
        if redemption_amount > note.outstanding_debt() {
            return Err(TransactionBuilderError::InsufficientFunds(format!(
                "Redemption amount {} exceeds outstanding debt {}",
                redemption_amount,
    ...
    /// Serialize bytes as Coll[Byte] constant (prefix 0x0e + 1-byte length + data)
    fn serialize_ergo_coll_bytes(data: &[u8]) -> String {
        ...
        format!("0e{:02x}{}", data.len(), hex::encode(data))
    }
    ...
    fn build_ergo_transaction_json(
        ...
        let ctx = tx_data.context_extension.as_ref().ok_or_else(|| {
            TransactionBuilderError::TransactionBuilding(
                "Context extension is required".to_string(),
            )
        })?;
        ...
        extension.insert(
            "1".to_string(),
            Self::serialize_ergo_group_element(&ctx.receiver_pubkey),
        );

        // #2: Reserve signature (Coll[Byte] constant, 65 bytes)
        extension.insert(
            "2".to_string(),
            Self::serialize_ergo_coll_bytes(&ctx.reserve_signature),
        );

        // #3: Total debt (Long constant)
        extension.insert(
            "3".to_string(),
            Self::serialize_ergo_long(ctx.total_debt as i64),
        );

        // #5: Insert proof (Coll[Byte] constant)
        extension.insert(
            "5".to_string(),
            Self::serialize_ergo_coll_bytes(&ctx.insert_proof),
        );

        // #6: Tracker signature (Coll[Byte] constant, 65 bytes)
        extension.insert(
            "6".to_string(),
            Self::serialize_ergo_coll_bytes(&ctx.tracker_signature),
        );
        ...
        extension.insert(
            "8".to_string(),
            Self::serialize_ergo_coll_bytes(&ctx.tracker_lookup_proof),
        );
        ...
        for fee_box_id in &tx_data.fee_input_box_ids {
        ...
        // Build outputs array: reserve output (index 0), recipient output (index 1),
        ...
        let mut outputs = vec![
            ...
            serde_json::json!({
                "value": tx_data.fee,
                // Fee recipient contract from the Scala reference implementation.
                // It allows anyone to spend the box (used by the miner).
        ...
        if change_amount > 0 {
            let change_address = tx_data.change_address.as_ref().ok_or_else(|| {
                TransactionBuilderError::Configuration(
                    "Change address required when fee inputs exceed fee".to_string(),
                )
            })?;
            let change_ergo_tree = address_to_ergo_tree(change_address).map_err(|e| {
                TransactionBuilderError::TransactionBuilding(format!(
...
fn address_to_ergo_tree(address_str: &str) -> Result<String, TransactionBuilderError> {
    ...
    let address = encoder.parse_address_from_str(address_str).map_err(|e| {
        TransactionBuilderError::Configuration(format!("Invalid address '{}': {}", address_str, e))
    })?;
...
mod tests {
    ...
    fn test_real_transaction_building() {
        ...
        // Verify inputs contain reserve box and one fee input
        let inputs = tx_json["tx"]["inputs"].as_array().unwrap();
        assert_eq!(inputs.len(), 2);
        assert_eq!(inputs[0]["boxId"], "test_reserve_box_1234567890abcdef");
        assert_eq!(inputs[1]["boxId"], "test_fee_input_1234567890abcdef");
    ...
    fn test_transaction_building_with_different_amounts() {
        ...
        for (amount, description) in test_cases {
            let tx_data = RedemptionTransactionData {
                ...
                recipient_address: "9fRusAarL1KkrWQVsxSRVYnvWxaAT2A96cKtNn9tvPh5XUyCisr33"
                    .to_string(),
                avl_proof: vec![0x01],
                issuer_signature: vec![0u8; 65],
                tracker_signature: vec![0u8; 65],
            ...
            assert!(
                ...
                "Transaction JSON missing 'tx' key for {}",
                description
            );
        }
    }
    ...
    fn test_transaction_building_with_different_fees() {
        ...
        for (fee, description) in test_cases {
            ...
            assert!(
                result.is_ok(),
                "Failed to build transaction with {}: {:?}",
                description,
                result.err()
            );
            let tx_bytes = result.unwrap();
            ...
            assert!(
                tx_json.get("tx").is_some(),
                "Transaction JSON missing 'tx' key with {}",
                description
            );
        }
    }
    ...
    fn test_transaction_building_error_conditions() {
        ...
        assert!(
            result.is_ok(),
            "Should build even with empty reserve box ID"
        );
    }
    ...
    fn test_transaction_building_with_test_helpers() {
        ...
        let tx_json: serde_json::Value =
            serde_json::from_slice(&tx_bytes).expect("Should be valid JSON");
        assert!(tx_json.get("tx").is_some());
        assert!(tx_json["tx"]["inputs"][0]["boxId"]
            .as_str()
            .unwrap()
            .contains("e56847ed"));
        assert!(tx_json["tx"]["dataInputs"][0]["boxId"]
            .as_str()
            .unwrap()
            .contains("f67858fe"));
    }
