...
fn main() {
    ...
    println!(
        "1. {} with {} nanoERG",
        reserve1.owner_pubkey, reserve1.base_info.collateral_amount
    );
    println!(
        "2. {} with {} nanoERG",
        reserve2.owner_pubkey, reserve2.base_info.collateral_amount
    );
    println!();
    ...
    println!(
        "Reserve 1 now has {} nanoERG debt",
        updated_reserve1.total_debt
    );
    println!(
        "Collateralization ratio: {:.2}",
        updated_reserve1.collateralization_ratio()
    );
    println!();
    ...
    println!(
        "  System collateralization ratio: {:.2}",
    ...
    println!(
        "Warning reserves (<= 125% collateralization): {}",
        warning_reserves.len()
    );
    println!(
        "Critical reserves (<= 100% collateralization): {}",
        critical_reserves.len()
    );
    println!();

    // Demonstrate reserve lookup by owner
    println!("Looking up reserve by owner public key...");
    let found_reserve = tracker
        .get_reserve_by_owner(&reserve2.owner_pubkey)
        .unwrap();
    println!(
        "Found reserve: {} with {} nanoERG",
        found_reserve.owner_pubkey, found_reserve.base_info.collateral_amount
    );
    println!();
