...
class BasisTokenSpec extends PropSpec with Matchers with ScalaCheckDrivenPropertyChecks with basis.HttpClientTesting {
  ...
  property("basis-token top-up should work") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val basisOutput = createOut(
        BasisConstants.basisTokenContract, minValue * 2,
  ...
  property("basis-token redemption should work with pending refund (R7 preserved)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val plasmaMap = new PlasmaMap[Array[Byte], Array[Byte]](basisReserveFlags, chainCashPlasmaParameters)
      val insertRes = plasmaMap.insertOrUpdate(key -> (Longs.toByteArray(timestamp) ++ Longs.toByteArray(redeemAmount)))
  ...
  property("basis-token redemption should fail with insufficient reserve token amount") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val plasmaMap = new PlasmaMap[Array[Byte], Array[Byte]](basisReserveFlags, chainCashPlasmaParameters)
      val insertRes = plasmaMap.insertOrUpdate(key -> (Longs.toByteArray(timestamp) ++ Longs.toByteArray(0L)))
      val insertProof = insertRes.proof
      ...
      val basisOutput = createOut(
        BasisConstants.basisTokenContract, minValue + feeValue - 100000000L,
  ...
  property("basis-token redemption should fail with invalid AVL tree proof") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val wrongInsertRes = wrongPlasmaMap.insertOrUpdate((wrongKey, Longs.toByteArray(timestamp) ++ Longs.toByteArray(0L)))
  ...
  property("basis-token redemption should fail with timestamp replay attack (old timestamp)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val oldPlasmaMap = new PlasmaMap[Array[Byte], Array[Byte]](basisReserveFlags, chainCashPlasmaParameters)
      oldPlasmaMap.insertOrUpdate(key -> (Longs.toByteArray(oldTimestamp) ++ Longs.toByteArray(totalDebt)))
  ...
  property("basis-token redemption should fail when redeeming more than debt delta") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val plasmaMap = new PlasmaMap[Array[Byte], Array[Byte]](basisReserveFlags, chainCashPlasmaParameters)
      plasmaMap.insertOrUpdate(key -> (Longs.toByteArray(timestamp) ++ Longs.toByteArray(redeemedDebt)))
      val inputTree = plasmaMap.ergoValue

      val insertRes = plasmaMap.insertOrUpdate(key -> (Longs.toByteArray(timestamp) ++ Longs.toByteArray(totalDebt)))
  ...
  property("BasisConstants.basisTokenContract matches ../contract/basis-token.es (text-equivalent)") {
    import java.nio.charset.StandardCharsets
    import java.nio.file.{Files, Paths}
    val projectRoot = Paths.get(sys.props("user.dir"))
    val contractPath = projectRoot.resolve("../contract/basis-token.es")
    ...
    BasisConstants.basisTokenContract shouldEqual fileText.replace("\r\n", "\n").stripSuffix("\n")
  }

  property("compiled basisTokenErgoTree matches fresh compilation of basis-token.es") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      val freshlyCompiled = ctx.compileContract(ConstantsBuilder.empty(), BasisConstants.basisTokenContract)
      BasisConstants.basisTokenErgoTree.bytes shouldEqual freshlyCompiled.getErgoTree.bytes
    }
  }
  ...
  property("basis-token multiple partial consecutive redemptions should work") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      val totalDebt = 1000000000L // 1 token unit total debt
      val redeem1 = 300000000L    // 0.3 token units first redemption
      val redeem2 = 400000000L    // 0.4 token units second redemption
      ...
      val basisInput1 = mkBasisTokenInput(
      ...
      val fundingBox2 = ctx.newTxBuilder().outBoxBuilder
        .value(minValue)
        .contract(ctx.compileContract(ConstantsBuilder.empty(), fakeScript))
        .build()
        .convertToInputWith(fakeTxId2, fakeIndex)
      ...
      val redemption2 = createOut(trueScript, minValue, Array(),
        Array(new ErgoToken(reserveTokenIdBytes, redeem2)))

      val tx2 = createTx(Array(basisInput2, fundingBox2), Array(trackerInput),
        Array(basisOutput2, redemption2), fee = None, changeAddress,
        Array(receiverSecret.toString()), broadcast = false)

      // --- Redemption 3: (t2, redeem1+redeem2) -> (t3, redeem1+redeem2+redeem3) ---
      ...
      val insertProof3 = updateMap3.insertOrUpdate(key -> (Longs.toByteArray(t3) ++ Longs.toByteArray(redeem1 + redeem2 + redeem3))).proof.bytes
      ...
      val fundingBox3 = ctx.newTxBuilder().outBoxBuilder
        .value(minValue)
        .contract(ctx.compileContract(ConstantsBuilder.empty(), fakeScript))
        .build()
      ...
      val redemption3 = createOut(trueScript, minValue, Array(),
        Array(new ErgoToken(reserveTokenIdBytes, redeem3)))

      noException should be thrownBy {
        createTx(Array(basisInput3, fundingBox3), Array(trackerInput),
