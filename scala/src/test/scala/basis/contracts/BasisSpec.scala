...
class BasisSpec extends PropSpec with Matchers with ScalaCheckDrivenPropertyChecks with basis.HttpClientTesting {

  implicit val addrEncoder = BasisConstants.ergoAddressEncoder
  ...
  val ownerPk = BasisConstants.g.exp(ownerSecret.bigInteger)
  val receiverSecret = SigUtils.randBigInt
  val receiverPk = BasisConstants.g.exp(receiverSecret.bigInteger)
  val trackerSecret = SigUtils.randBigInt
  val trackerPk = BasisConstants.g.exp(trackerSecret.bigInteger)
  ...
  def trueErgoTree = BasisConstants.compile(trueScript)
  def trueErgoContract = new ErgoTreeContract(trueErgoTree, BasisConstants.networkType)

  def createOut(contract: String,
                value: Long,
                registers: Array[ErgoValue[_]],
                tokens: Array[ErgoToken])(implicit ctx: BlockchainContext): OutBoxImpl = {
    val c = ErgoScriptContract.create(new org.ergoplatform.appkit.Constants, contract, BasisConstants.networkType, 4: Byte)
  ...
  val refundPeriod = 43200L // must match refundPeriod in basis.es

  property("basis initiate refund should work") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val basisInput =
        ctx
          ...
          .convertToInputWith(fakeTxId1, fakeIndex)
          .withContextVars(
            new ContextVar(0, ErgoValue.of(20: Byte)) // action 2, index 0
          )
      ...
      noException should be thrownBy {
        createTx(
          Array[InputBox](basisInput, fundingBox),
          Array[InputBox](),
          Array[OutBoxImpl](basisOutput),
          fee = None,
          changeAddress,
          Array[String](ownerSecret.toString()),
          false
  ...
  property("basis initiate refund should fail without owner signature") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
  ...
  property("basis initiate refund should fail when refund is already pending") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      assertTxFails(
        Array(basisInput, fundingBox),
        Array(),
        Array(basisOutput),
        Array(ownerSecret.toString())
      )
    }
  }

  property("basis initiate refund should fail with backdated initiation height") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
  ...
  property("basis initiate refund should fail when taking funds out") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      // Output value decreased (fee is paid from the reserve) - not allowed at initiation
      val basisOutput = createOut(
        BasisConstants.basisContract,
        minValue - feeValue,
        Array(ErgoValue.of(ownerPk), emptyTreeErgoValue, ErgoValue.of(trackerNFTBytes), ErgoValue.of(ctx.getHeight.toLong)),
  ...
  property("basis complete refund should work after waiting period") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val basisInput =
        ctx
          ...
          .convertToInputWith(fakeTxId1, fakeIndex)
          .withContextVars(
            new ContextVar(0, ErgoValue.of(30: Byte)) // action 3, index 0
          )

      // Refund output: all funds and the reserve token leave the contract
  ...
  property("basis complete refund should fail before waiting period is over") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
  ...
  property("basis complete refund should fail without pending refund") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
  ...
  property("basis complete refund should fail without owner signature") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      a[Throwable] should be thrownBy {
        createTx(
          Array[InputBox](basisInput),
          Array[InputBox](),
          Array[OutBoxImpl](refundOutput),
          fee = None,
          changeAddress,
          Array[String](receiverSecret.toString()),
          false
  ...
  property("basis redemption should work with pending refund (R7 preserved)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val TreeAndProof(initialTree, nextTree, proofBytes) = mkTreeAndProof(key, redeemAmount, timestamp)
      val TrackerTreeAndProof(trackerTree, trackerLookupProof) = mkTrackerTreeAndProof(key, totalDebt)
      ...
      val basisInput = mkBasisInput(minValue + totalDebt + feeValue, initialTree,
        ...
        Some(trackerLookupProof), timestamp, Some(refundHeight))
      val trackerDataInput = mkTrackerDataInput(trackerTree)

      // R7 preserved in the reserve output
      val basisOutput = createOut(BasisConstants.basisContract, minValue + feeValue,
        Array(ErgoValue.of(ownerPk), nextTree, ErgoValue.of(trackerNFTBytes), ErgoValue.of(refundHeight)),
        Array(new ErgoToken(basisTokenId, 1)))
  ...
  property("basis redemption should fail when clearing pending refund flag") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      val totalDebt = 500000000L
      val redeemAmount = totalDebt
      val timestamp = System.currentTimeMillis()
  ...
  property("basis top-up should work with pending refund (R7 preserved)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
  ...
  property("basis redemption should fail with invalid action code") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      // Basis reserve input with invalid action code (5)
      val basisInput =
        ctx
          .newTxBuilder()
          .outBoxBuilder
          ...
          .convertToInputWith(fakeTxId1, fakeIndex)
          .withContextVars(
            new ContextVar(0, ErgoValue.of(50: Byte)), // Invalid action code 5
            new ContextVar(1, ErgoValue.of(receiverPk)),
            new ContextVar(2, ErgoValue.of(reserveSigBytes)),
            new ContextVar(3, ErgoValue.of(debtAmount)),
            new ContextVar(4, ErgoValue.of(timestamp)),
  ...
  property("BasisConstants.basisContract matches ../contract/basis.es (text-equivalent)") {
    ...
    // BasisConstants.readContract normalizes to \n via getLines.mkString("\n")
    BasisConstants.basisContract shouldEqual fileText.replace("\r\n", "\n").stripSuffix("\n")
  }

  property("compiled basisErgoTree matches fresh compilation of basis.es") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      val freshlyCompiled = ctx.compileContract(ConstantsBuilder.empty(), BasisConstants.basisContract)
      BasisConstants.basisErgoTree.bytes shouldEqual freshlyCompiled.getErgoTree.bytes
    }
  }
  ...
  property("basis redemption should fail with different contract in output (with control)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val goodBasisOutput = createOut(BasisConstants.basisContract, minValue + feeValue,
        Array(ErgoValue.of(ownerPk), nextTree, ErgoValue.of(trackerNFTBytes)),
        Array(new ErgoToken(basisTokenId, 1)))
      noException should be thrownBy {
        assertTxSucceeds(Array(basisInput), Array(trackerDataInput),
  ...
  property("basis redemption should fail with missing token in output (with control)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val badBasisOutput = createOut(BasisConstants.basisContract, minValue + feeValue,
        Array(ErgoValue.of(ownerPk), nextTree, ErgoValue.of(trackerNFTBytes)),
  ...
  property("basis redemption should fail with changed owner key in output (with control)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val differentOwnerPk = BasisConstants.g.exp(SigUtils.randBigInt.bigInteger)
  ...
  property("basis top-up should fail with changed tree (with control)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val badBasisOutput = createOut(BasisConstants.basisContract, minValue + topUpAmount,
      ...
      val goodBasisOutput = createOut(BasisConstants.basisContract, minValue + topUpAmount,
        Array(ErgoValue.of(ownerPk), emptyTreeErgoValue, ErgoValue.of(trackerNFTBytes)),
        Array(new ErgoToken(basisTokenId, 1)))
      noException should be thrownBy {
  ...
  property("debt transfer: triangular trade with consent should work") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val carolKey = BasisConstants.g.exp(carolSecret.bigInteger)
      ...
      val basisOutputBob = createOut(BasisConstants.basisContract,
      ...
      val basisOutputCarol = createOut(BasisConstants.basisContract,
  ...
  property("debt transfer: multi-hop triangular trade concept verification") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val daveKey = BasisConstants.g.exp(daveSecret.bigInteger)
      ...
      val basisOutputBob = createOut(BasisConstants.basisContract, reserveBob - debtToBob,
  ...
  property("debt transfer: should fail with replay attack (reuse old note)") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val trackerInputBob = mkTrackerDataInput(trackerTreeBob.tree)
      val basisOutputBob = createOut(BasisConstants.basisContract, reserveBob - bobRemaining,
        Array(ErgoValue.of(aliceKey), treeBobOut, ErgoValue.of(trackerNFTBytes)),
        Array(new ErgoToken(basisTokenId, 1)))
      ...
      val basisOutputReplay = createOut(BasisConstants.basisContract, reserveBob2,
  ...
  property("debt transfer: should fail with insufficient collateral") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val basisOutputBob = createOut(BasisConstants.basisContract, insufficientReserve - bobRemaining, // Would be negative!
        Array(ErgoValue.of(aliceKey), treeBobOut, ErgoValue.of(trackerNFTBytes)),
        Array(new ErgoToken(basisTokenId, 1)))
      val redemptionBob = createOut(trueScript, bobRemaining, Array(), Array())
  ...
  property("basis multiple partial consecutive redemptions should work") {
    createMockedErgoClient(MockData(Nil, Nil)).execute { implicit ctx: BlockchainContext =>
      ...
      val basisOutput1 = createOut(BasisConstants.basisContract,
      ...
      val basisOutput2 = createOut(BasisConstants.basisContract,
      ...
      val basisOutput3 = createOut(BasisConstants.basisContract,
