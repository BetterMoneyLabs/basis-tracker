...
class BasisDeployerSpec extends PropSpec with Matchers {
  ...
  property("BasisDeployer should compile Basis contract successfully") {
    ...
    val basisContract = BasisConstants.readContract("basis.es", Map.empty)
    basisContract should not be empty

    val basisErgoTree = BasisConstants.compile(basisContract)
    ...
    BasisConstants.ergoAddressEncoder.fromString(basisAddress.toString).get shouldEqual basisAddress
  }
